...
const AdminCard = ({ ticket, onDelete, onUpdate }) => {
  ...
  return (
    <div
      className={`relative border rounded-2xl p-5 shadow-lg hover:shadow-2xl transition duration-300 bg-white flex flex-col gap-3 w-full lora h-full min-h-[300px] max-h-[400px] overflow-hidden ${getBorderColor(
        ticket.priority
      )} cursor-pointer`}
    >
      {/*  Correct place to render logo */}
      {getLogo(ticket.status)}

      <h3 className="lg:text-2xl font-bold text-gray-800 truncate md:text-xl text-lg">
        {ticket.title}
      </h3>
      <p className="text-gray-900 line-clamp-3 md:text-sm truncate">
        {ticket.description}
      </p>

      <div className="flex justify-between flex-wrap gap-2 mt-2">
        <span
          className={`px-3 py-1 rounded-full font-semibold ${getPriorityColor(
            ticket.priority
          )} md:text-[15px]`}
        >
          {ticket.priority}
      ...
      <div className="flex flex-col md:text-[15px] text-xs">
        ...
        <span className="flex gap-1">
          USER ID: {ticket.id}
          <button onClick={() => copy(ticket.id)} className="cursor-pointer">
            <svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-copy-icon lucide-copy"><rect width="14" height="14" x="8" y="8" rx="2" ry="2" /><path d="M4 16c-1.1 0-2-.9-2-2V4c0-1.1.9-2 2-2h10c1.1 0 2 .9 2 2" /></svg>
          </button>
        </span>


        <span>USERNAME: {ticket.user}</span>
